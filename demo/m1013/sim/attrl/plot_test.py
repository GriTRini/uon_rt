...
# C++ 코드에서 제공해주신 링크 구조 (Offset) 설정
LINK_OFFSETS = [
    get_transform_matrix(0, 0, 0.1525, 0, 0, 0),                           # Base -> Link1
    get_transform_matrix(0, 0.0345, 0, 0, -np.pi/2, -np.pi/2),             # Link1 -> Link2
    get_transform_matrix(0.62, 0, 0, 0, 0, np.pi/2),                       # Link2 -> Link3
    get_transform_matrix(0, -0.559, 0, np.pi/2, 0, 0),                     # Link3 -> Link4
    get_transform_matrix(0, 0, 0, -np.pi/2, 0, 0),                         # Link4 -> Link5
    get_transform_matrix(0, -0.121, 0, np.pi/2, 0, 0)                      # Link5 -> Link6
]
...
def plot_robot_trajectory_3d(file_path="reach_and_lift_test.csv"):
    ...
    # ------------------ [우측 차트] 기존 Doosan M1013 3D 로봇 모델 시각화 ------------------
    ...
    line_arm, = ax_3d.plot(joints[:, 0], joints[:, 1], joints[:, 2], 
                           color='dodgerblue', linewidth=5, marker='o', markersize=8, markerfacecolor='orange', label='M1013 Arm')

    ax_3d.legend()
    ax_3d.set_title('Doosan M1013 Kinematics Simulation', fontsize=13, fontweight='bold')
    ax_3d.set_xlabel('X (m)')
    ax_3d.set_ylabel('Y (m)')
    ax_3d.set_zlabel('Z (m)')

    # 로봇의 전체 가동 범위를 고려한 3D 가상 공간 축 스케일 고정
    ax_3d.set_xlim([-1.0, 1.5])

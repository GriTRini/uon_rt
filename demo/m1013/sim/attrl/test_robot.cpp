...
int main() {
    ...
    // ==========================================================
    std::cout << "\nStep 2: Move to X=0.0, Y=1.1, Z=0.0" << std::endl;
    
    tmat_t target_tmat = traj_gen.tmat(); // 현재 TCP 회전 행렬 복사
    target_tmat.translation() << 0.0, 1.1, 0.0; // 1.1m로 무리한 병진 좌표 설정
    
    // 🌟 [안전 예외 처리 적용] attrl의 반환값을 확인합니다.
    if (!traj_gen.attrl(target_tmat, 500.0)) {
        ...
        std::cout << "✅ [SUCCESS] 명령 수락됨: Y=1.1m 위치로 이동을 시작합니다." << std::endl;
    }

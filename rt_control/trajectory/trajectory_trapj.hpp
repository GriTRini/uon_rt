...
public:
    ...
    void update(value_t dt) {
        ...
        for (int i = 0; i < 6; ++i) {
            value_t q, dq, ddq;
            calculate_state(i, m_time, q, dq, ddq);
            m_angles(i) = q;
            m_angvels(i) = dq;
            m_angaccs(i) = ddq;
        }
...
private:
    ...
    void calculate_state(int i, value_t t, value_t& x, value_t& v, value_t& a) {
        ...
        if (t < p.t_acc) {
            x = m_q_start(i) + m_dq_start(i) * t + 0.5 * p.a_acc * t * t;
            v = m_dq_start(i) + p.a_acc * t;
            a = p.a_acc;
        } else if (t < p.t_acc + p.t_const) {
            value_t dt = t - p.t_acc;
            value_t x1 = m_q_start(i) + m_dq_start(i) * p.t_acc + 0.5 * p.a_acc * p.t_acc * p.t_acc;
            x = x1 + p.v_const * dt;
            v = p.v_const;
            a = 0.0;
        } else {
            value_t dt = t - (p.t_acc + p.t_const);
            value_t x2 = m_q_start(i) + m_dq_start(i) * p.t_acc + 0.5 * p.a_acc * p.t_acc * p.t_acc + p.v_const * p.t_const;
            x = x2 + p.v_const * dt + 0.5 * p.a_dec * dt * dt;
            v = p.v_const + p.a_dec * dt;
            a = p.a_dec;
        }
    }

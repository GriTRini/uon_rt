...
def main():
    ...
    NUM_OF_MOTIONS = 4
    mid_pos = [[0 for _ in range(3)] for _ in range(NUM_OF_MOTIONS)]
    end_pos = [[0 for _ in range(3)] for _ in range(NUM_OF_MOTIONS)]
    end_ort = [[0 for _ in range(3)] for _ in range(NUM_OF_MOTIONS)]
    ...
    for iter in range(0, 10):
        ...
        for i in range(0, NUM_OF_MOTIONS):
            ...
            err_ret_val = clink_rpc_api.clink_rpc_motion_command_robot_tcp_position_mid_set(cbox_id, cmd_id, mid_pos[i][0], mid_pos[i][1], mid_pos[i][2])
            err_ret_val = clink_rpc_api.clink_rpc_motion_command_robot_tcp_position_end_set(cbox_id, cmd_id, end_pos[i][0], end_pos[i][1], end_pos[i][2])
            err_ret_val = clink_rpc_api.clink_rpc_motion_command_robot_tcp_orientation_end_set(cbox_id, cmd_id, end_ort[i][0], end_ort[i][1], end_ort[i][2])
            err_ret_val = clink_rpc_api.clink_rpc_motion_command_robot_tcp_interpolator_set(cbox_id, cmd_id, clink_rpc_api.CLINK_INTERPOLATOR_CIRCULAR)

            # CLINK_INTERPOLATOR_CIRCULAR 선택 시 추가 입력
            err_ret_val = clink_rpc_api.clink_rpc_motion_command_robot_tcp_radius_set(cbox_id, cmd_id, 50)

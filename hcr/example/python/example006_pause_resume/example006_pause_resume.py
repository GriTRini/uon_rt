...
def main():
    # pause(일시정지) 및 resume(재시작) 명령이 가능하다.
    ...
    for j_idx in range(0, NUM_OF_JOINTS):
        err_ret_val = clink_rpc_api.clink_rpc_motion_command_robot_joint_angle_end_set(cbox_id, cmd_id, j_idx, angles[0][j_idx])
    ...
    err_ret_val = clink_rpc_api.clink_rpc_motion_command_robot_robot_id_set(cbox_id, cmd_id, robot_id)
    for j_idx in range(0, NUM_OF_JOINTS):
        err_ret_val = clink_rpc_api.clink_rpc_motion_command_robot_joint_angle_end_set(cbox_id, cmd_id, j_idx, angles[1][j_idx])
    ...
    for i in range(0, 4):
        err_ret_val = clink_rpc_api.clink_rpc_robot_pause(cbox_id, robot_id, 0.5)
        time.sleep(2)
        err_ret_val = clink_rpc_api.clink_rpc_robot_resume(cbox_id, robot_id)
        time.sleep(2)

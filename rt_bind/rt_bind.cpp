...
PYBIND11_MODULE(rt_control_cpp_impl, m) {
    ...
    // ---------------------------------------------------------
    // 3. [Sim] TrajGenerator (시뮬레이션 전용 어댑터)
    // ---------------------------------------------------------
    ...
    // ---------------------------------------------------------
    // 4. [Real] RobotBase (실제 하드웨어 연동 공통 인터페이스)
    // ---------------------------------------------------------
    py::class_<rt_control::RobotBase, std::unique_ptr<rt_control::RobotBase>>(m, "RobotBase")
        ...
        .def("attrl", [](rt_control::RobotBase& self, const py::array_t<double>& goal_tmat, double kp) {
